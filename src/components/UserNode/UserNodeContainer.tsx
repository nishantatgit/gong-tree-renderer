...
function UserNodeContainer({
    ...
    const user = useAppSelector((state) => state.hierarchy.entities[userId])

    const isExpanded = useAppSelector((state) =>
        Boolean(state.expansion.expandedById[userId]),
    )
    ...
    return (
        <UserNode
            ...
            onToggle={() => dispatch(toggleNode(userId))}
            depth={depth}
        >
            {isExpanded && user.children.length > 0 && (
                <ul role="group">
                    {user.children.map((childId: number) => (
                        <UserNodeContainer
                            key={childId}
                            userId={childId}
                            depth={depth + 1}
